    ...
    <div className="page-enter space-y-10">
      ...
      <div className="glass p-6">
        ...
        <div className="mt-4 flex gap-3">
          ...
          <button onClick={() => {
            const summary = `CreatorBloom Media Kit\n========================\n\nAvg. Video Views: 142K\nEngagement Rate: 4.8%\nAudience Demo: 18-34 / 62% M\n\nGenerated by CreatorBloom — creatorbloom.app`
            const blob = new Blob([summary], { type: 'text/plain' })
            const url = URL.createObjectURL(blob)
            const a = document.createElement('a')
            a.href = url; a.download = 'media-kit.txt'; a.click()
            URL.revokeObjectURL(url)
          }} className="px-4 py-2 text-sm font-medium bg-accent-600/15 hover:bg-accent-600/25 text-accent-400 rounded-xl transition-colors">Download Media Kit</button>
        </div>
      </div>
